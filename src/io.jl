...
function readdssp(dssp_filepath::AbstractString)
    ...
    reading = false
    open(dssp_filepath, "r") do dssp_file
...
function readpdb(in_filepath::AbstractString; hetatm::Bool=false)
    @assert isfile(in_filepath) "Not a valid filepath: \"$in_filepath\""
    atoms = Atom[]
...
"""
Space an atom name correctly in the four-character bounds.
...
function writeensemblescores(out_filepath::AbstractString, ensemble::ModelledEnsemble)
    ...
    println("Wrote ensemble scores to file \"$out_filepath\"")
end
...
function writeprojections(out_filepath::AbstractString, pcs::Array{Float64}, pcs_to_write::Array{Int,1}=collect(1:size(pcs, 1)))
    ...
    println("Projections written to file \"$out_filepath\"")
end
...
function writepcviews(out_prefix::AbstractString,
    ...
    for i in 1:n_to_use
        out_filepath = "$out_prefix$i.pml"
        writepcview(out_filepath, pca, atoms, i)
    end
    println("PyMol script to view principal components written to file(s) \"$(out_prefix)x.pml\"")
end
...
function readpocketpoints(pdb_filepath::AbstractString)
    ...
    open(pdb_filepath, "r") do pdb_file
